let productTemplate = (prod, index) => {

    let description = prod.description.replace("\n", "<br>").replace("###", "");
    let item = `
    <div class="product" >
        <div class="image" style="
        background-image: url('https://imagedelivery.net/95QNzrEeP7RU5l5WdbyrKw/${prod.cloudflare_image_id}/shopitem');
        ...
        <button data-sellix-product="${prod.uniqid}" >BUY</button>
...
let getProducts = () => {
    ...
    .then(response =>{
        ...
        products.map(prod => {
            if(prod.stock != 0 && !prod.unlisted){
                $(".products").append(productTemplate(prod))
            }
        })
